-- LegalMind Yemen - production firm code system

create extension if not exists "pgcrypto";

alter table firms add column if not exists firm_code varchar(12);

create unique index if not exists firms_firm_code_unique_idx
  on firms(firm_code);
...
begin
  for firm_row in
    ...
    where firm_code is null
       or firm_code !~ '^[A-Z]{3}-[0-9]{4}$'
  loop
    code := case
      when firm_row.firm_code ~ '^[A-Z]{3}-[0-9]{4}$' then firm_row.firm_code
      else generate_firm_code(firm_row.name)
    end;

    update firms
    set firm_code = code
    where id = firm_row.id;
  end loop;
end $$;
