-- Aggressive office-owner profile repair: relink orphaned employees, match firm/employee email
...
begin
  ...
  -- 1) Employee already linked to this auth user
  ...
  -- 2) Employee with same email (relink orphaned / stale auth_uid)
  if not found then
    select e.* into v_employee
    from public.employees e
    ...
    if found and v_employee.auth_uid is distinct from v_uid then
      ...
      set auth_uid = v_uid, status = 'active'
  ...
  -- 3) Firm matched by email (create owner employee)
  ...
  -- 4) Auth metadata indicates office registration
  ...
  -- 6) Last resort: provision new office for authenticated user with unused email
  if not exists (
